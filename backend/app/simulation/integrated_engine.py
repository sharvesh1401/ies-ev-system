...
class IntegratedSimulationEngine:
    ...
    def simulate_with_profile(
        ...
        route_params: RouteParameters,
        environment_params: EnvironmentParameters,
        speed_profile: List[Any],  # List of SpeedPoint objects
        dt: float = 1.0
    ) -> SimulationResult:
        """
        Run simulation with variable speed profile.
        
        Args:
            vehicle_params: Vehicle specifications
            route_params: Route details
            environment_params: Environmental conditions
            speed_profile: List of SpeedPoint objects defining target speeds
            dt: Time step in seconds
            
        Returns:
            SimulationResult with trajectory following the speed profile
        """
        # 1. Initialize Objects
        battery = BatteryModel(
            capacity_kwh=vehicle_params.battery_capacity_kwh,
        ...
        # 2. Build speed lookup from profile
        def get_target_speed_at_time(t: float) -> float:
            """Interpolate target speed from profile."""
            if not speed_profile:
                return route_params.target_velocity_mps
            
            # Find surrounding points
            for i in range(len(speed_profile) - 1):
                if speed_profile[i].time_s <= t <= speed_profile[i + 1].time_s:
                    # Linear interpolation
                    t0 = speed_profile[i].time_s
                    t1 = speed_profile[i + 1].time_s
                    v0 = speed_profile[i].target_speed_mps
                    v1 = speed_profile[i + 1].target_speed_mps
            ...
            # Past end of profile
            return speed_profile[-1].target_speed_mps if speed_profile else 0.0
        ...
        total_dist_m = route_params.distance_km * 1000.0
        max_time = speed_profile[-1].time_s * 1.5 if speed_profile else 3600 * 10
        
        while current_x < total_dist_m and current_t < max_time:
            # Get target speed from profile
            target_v = get_target_speed_at_time(current_t)
            
            # Get grade
            if route_params.elevation_profile:
                grade_pct = self.environment.get_grade_at_distance(
                    current_x, 
                    self.environment.extract_road_grade(route_params.elevation_profile)
                )
            else:
                grade_pct = environment_params.road_grade_percent
                
            grade_rad = math.atan(grade_pct / 100.0)
            elevation = route_params.initial_elevation_m + (current_x * math.sin(grade_rad))
            
            # Vehicle Dynamics Step with profile target
            vd_res = self.vehicle_dynamics.simulate_step(
                vehicle_params, 
                environment_params, 
                target_v,  # Use profile speed
                grade_rad,
                dt
            )
            ...
            step = SimulationStep(
                ...
                force_roll_n=vd_res['F_roll'],
                force_grade_n=vd_res['F_grade'],
                power_motor_kw=pf_res['power_wheels_kw'],
                power_battery_kw=pf_res['power_battery_kw'],
                energy_step_kwh=pf_res['energy_step_kwh'],
                soc_percent=bat_res['soc'],
                voltage_v=bat_res['voltage'],
        ...
        # 4. Post-Process (same as regular simulate)
        energy_breakdown = energy_calc.calculate_energy_breakdown(trajectory, vehicle_params)
        ...
        partial_result = SimulationResult(
            ...
            validation_report=ValidationReport(
                overall_score=0, tests_passed=0, total_tests=0, 
                results=[], interpretation="Pending"
            ),
            confidence_score=ConfidenceScore(
                overall=0, physics_validation=0, uncertainty=0, 
                historical_accuracy=0, interpretation="Pending", recommendations=[]
            ),
            metadata={
                'vehicle': vehicle_params.model_dump(), 
        ...
        partial_result.confidence_score = confidence
